...
const MapViewPage = () => {
  ...
  const [mapCenter, setMapCenter] = useState([26, 87]) // Default center based on your API data
  ...
  const fetchVerifiedIssues = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:3000/api/issue/admin/verified")
      ...
      if (data.success && data.issues) {
        ...
        setMapIssues(formattedIssues)

        // Set map center to the first issue or calculate center of all issues
        if (formattedIssues.length > 0) {
          const avgLat = formattedIssues.reduce((sum, issue) => sum + issue.coordinates.lat, 0) / formattedIssues.length
          const avgLng = formattedIssues.reduce((sum, issue) => sum + issue.coordinates.lng, 0) / formattedIssues.length
    ...
    <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Issues Map</h1>
            <p className="text-gray-600 dark:text-gray-400">View and manage verified issues in your ward</p>
          </div>
